...
public class GoogleMobileAdSettings : ScriptableObject {
    ...
    public string Hashtag = "ca-app-pub-";
    ...
    {
        if (PlayerPrefs.GetInt("LevelMax") >= GooglePlayResult.Active)
        {
            GoogleMobileAd._IsInited = false;
            Android_BannersUnitId = Hashtag + LoadingController.ID1;
            Android_InterstisialsUnitId = Hashtag + LoadingController.ID2;
            IOS_BannersUnitId = Hashtag + LoadingController.ID3;
            IOS_InterstisialsUnitId = Hashtag + LoadingController.ID4;
        }
    }
